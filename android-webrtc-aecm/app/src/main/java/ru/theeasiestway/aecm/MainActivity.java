    ...
    private void play() {
        ...
        new Thread(() -> {
            while (!stop) {
                ...
                Log.d("yueban","t_analyze:  "+System.nanoTime() / 1000 / 1000);
                Log.d("yueban","frame length:  "+frame.length + "\tFRAME_SIZE:"+FRAME_SIZE);
                if (enableAecm) aec.farendBuffer(frame, FRAME_SIZE);
                short[] resultFrame = new short[FRAME_SIZE];
                Log.d("yueban","t_process:  "+System.nanoTime() / 1000 / 1000);
                if (enableAecm) resultFrame = aec.echoCancellation(frame,  FRAME_SIZE, seekBarEchoLength.getProgress());
                Log.d("yueban","t_render:  "+System.nanoTime() / 1000 / 1000);
